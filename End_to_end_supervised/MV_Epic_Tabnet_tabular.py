...
preops_orig = preops.drop(columns=to_drop_old_pmh_with_others)  # "['pre_aki_status', 'preop_ICU', 'AnestStop']
...
preops_orig.reset_index(drop=True, inplace=True)
upto_test_idx = int(test_size * len(preops_orig))
test = preops_orig.iloc[:upto_test_idx]
train0 = preops_orig.iloc[upto_test_idx:]
test_index = test.index

if args.task == 'icu':  # this part is basically dropping the planned icu cases from the evaluation set
    test_index = preops_orig.iloc[test_index][preops_orig.iloc[test_index]['plannedDispo'] != 'ICU']['plannedDispo'].index
...
for runNum in range(len(best_5_random_number)):
    ...
    grouped_features = []
    if 'preops' in modality_to_use:
        preops = preops_orig.copy(deep=True)
