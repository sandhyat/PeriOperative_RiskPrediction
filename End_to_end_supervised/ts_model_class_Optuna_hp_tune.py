...
This model class file is same as the 'preop_flow_med_bow_model.py' in terms of architecture.
The only difference is that this file uses optuna so that the HP optimization can be performed.
        ...
        if 'pmh' in self.modality_to_use:
            ...
            if self.Att_pmh_Agg == True:
                ...
                pmh_att = torch.cat([self.class_token.expand(bs, -1, -1), pmh],
                                          dim=1)
                encodings, _ = self.attention_pmh(pmh_att, pmh_att,pmh_att)
