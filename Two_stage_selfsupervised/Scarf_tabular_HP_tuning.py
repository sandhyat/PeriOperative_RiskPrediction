...
def objective(trial, args):
    ...
    batchSize = trial.suggest_int("batchSize", 256, 1024)
    dim_hidden_encoder = trial.suggest_int("dim_hidden_encoder", 5,16) # hidden units in each layer
    num_hidden_encoder = trial.suggest_int("num_hidden_encoder", 2, 8) # depth of the encoder
    dim_hidden_head = trial.suggest_int("dim_hidden_head", 8,36) # hidden units in each layer
    num_hidden_head = trial.suggest_int("num_hidden_head", 2,6) # depth of the pretraining head
    corruption_rate = trial.suggest_float("corruption_rate", 0.3, 0.8)
    dropout = trial.suggest_float("dropout", 0.01, 0.3, log=False)
