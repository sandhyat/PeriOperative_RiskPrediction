...
if 'preops' in modality_to_use:
    ...
    preops_tr, preops_val, preops_te, train_index, valid_index, test_index, preops_mask = preprocess_train(preops,
                                                                                                               args.task,
                                                                                                               y_outcome=
                                                                                                               outcome_df[
                                                                                                                   "outcome"].values,
...
for epoch in range(1, args.epochs + 1):
    ...
    if epoch % 10 == 0:
        print(f"epoch {epoch}/{args.epochs} - loss: {loss_history[-1]:.4f}", end="\r")


# calculating embedding for downstream task
train_loader = DataLoader(train_ds, batch_size=args.batchSize, shuffle=False)
valid_loader = DataLoader(valid_ds, batch_size=args.batchSize, shuffle=False)
...
xgb_model = XGBClassifier(random_state=args.randomSeed)
clf = GridSearchCV(xgb_model,{"max_depth": [4, 6], "n_estimators": [50, 100, 200], "learning_rate": [0.01, 0.1, 1.0]}, cv=3,verbose=1,)
clf.fit(train_embeddings, y_train)

y_score = clf.best_estimator_.predict_proba(test_embeddings)  # this is from the non-calibrated classifier
y_score_tr = clf.best_estimator_.predict_proba(train_embeddings)  # this is from the non-calibrated classifier
auprc = average_precision_score(y_test, y_score[:, 1])
auroc = roc_auc_score(y_test, y_score[:, 1])

test_eval_result = {'auprc': auprc, 'auroc': auroc}
